...
problems_submitted = []   # List of submissions containg a string concatenated by contest ID and Question Key
...
if parsed['status'] == 'OK':
    for i in range(len(parsed['result'])):
        if (str(parsed['result'][i]['verdict']) == 'OK'):
            problems_submitted.append(str(parsed['result'][i]['problem']['contestId']) + str(parsed['result'][i]['problem']['index']))

    for problem in problems_submitted:
        if (problem == input_problem):
